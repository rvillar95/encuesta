            ...
            <form method="post" id="import_form" enctype="multipart/form-data">
                <div class="input-group">
                    ...
                    <div class="input-group-append">
                        <input type="submit" id="imp" name="import" value="Importar Usuarios" class="btn btn-outline-secondary" />
                    </div>
                </div>
            </form>
        </div>
...
<div class="modal" id="modalCuestionarios" tabindex="-1">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Modal Cuestionarios</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
                    ...
                    <div class="col-md-12">
                        <div class="table-responsive">
                            <table class="table table-striped " width="100%">
                                ...
                                <tbody id="tbodyCuestionarios"></tbody>
                            </table>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    $("body").on("click", "#verCuestionarios", function(e) {
        e.preventDefault();
        let id = $(this).val();
        ...
        $.ajax({
            ...
            console.table(msg.mensaje);
            let fila = "";
            $("#tbodyCuestionarios").empty();
            if (msg.resultado == "1") {

                msg.mensaje.forEach(element => {
                    fila += '<tr>';
                    fila += '   <td>' + element.fecha + '</td>';
                    fila += '   <td>' + element.fecha_uso + '</td>';
                    fila += '   <td>' + element.fecha_termino + '</td>';
                    if (element.estado == "U") {
                        fila += '   <td><span class="badge badge-warning">En Curso</span></td>';
                    } else if (element.estado == "A") {
                        fila += '   <td><span class="badge badge-danger">Sin Iniciar</span></td>';
                    } else if (element.estado == "F") {
                        fila += '   <td><span class="badge badge-success">Finalizado</span></td>';
                    }
                    fila += '   <td><button class="btn btn-info" id="iraldetalle" value="' + element.id + '" >Ver Detalle Cuestionario</button></td>';

                    fila += '</tr>';
                });
                $("#tbodyCuestionarios").append(fila);

            } else {
                toastr.error(msg.mensaje);
            }
        });
        $("#modalCuestionarios").modal("show");
    });

    $("body").on("click", "#iraldetalle", function(e) {
        e.preventDefault();
        let token = $(this).val();
        window.location.href = "detalle_encuesta?token=" + token;
    });
    ...
    $("body").on("click", "#btnVerCuestionario", function(e) {
        e.preventDefault();
        let partes = $(this).val().split(',');
        let id = partes[0];
        let estado = partes[1];
